...
```bash
helm install expanso-edge-compute oci://quay.io/expanso/helm/expanso-edge-compute
```

### From Local Chart

```bash
helm install expanso-edge-compute ./expanso-edge-compute
```
...
```bash
helm install expanso-edge-compute ./expanso-edge-compute -f values.yaml
```
...
```bash
helm upgrade expanso-edge-compute ./expanso-edge-compute \
  --reuse-values \
  --version 1.1.0
```
...
```bash
helm uninstall expanso-edge-compute
```
...
Check pod status:
```bash
kubectl get pods -l app.kubernetes.io/name=expanso-edge-compute
kubectl describe pod -l app.kubernetes.io/name=expanso-edge-compute
```

Check logs:
```bash
kubectl logs -l app.kubernetes.io/name=expanso-edge-compute
```
...
```bash
oc expose svc/expanso-edge-compute
```
