...
After merging your pull request, one of two paths are taken:
1. By default:
   - the CI/CD pipeline will publish the chart in the GitHub releases and add it to the Helm [charts repository index][index-url].
...
1. For provider controlled delivery:
  - The submission must be report only and provider delivery must be set both in the OWNERs file for the chart and the submitted report. The chart download URL and metadata are added to an unpublished index onlu y and no other information is published. In this case the chart URL should be a valid URL but does not need to be publically accessible.
...
* [OpenShift Helm Charts Repository](#openshift-helm-charts-repository)
   ...
   * [Troubleshooting Pull Request Failures](#troubleshooting-pull-request-failures)
      ...
      * [Provider controlled delivery errors](#provider-controlled-delivery=errors)  
...
2. Submit a Chart Verification Report without the Chart
   ...
   - Chart url must point to the downloadable location of the chart.
...
### Provider controlled delivery errors

For provider controlled delivery: 
- the submission must be report only. 
- provider controlled delivery must be set in both the OWNERS file and the submitted report.

If these condition are not met you will see these kind of error messages:

```
[ERROR] OWNERS file and/or report indicate provider controlled delivery but pull request is not report only.
[ERROR] Report indicates provider controlled delivery but OWNERS file does not.
[ERROR] OWNERS file indicates provider controlled delivery but report does not.
```

See also: [setting the provider controlled delivery in the verification repoort][provider controlled delivery](https://github.com/redhat-certification/chart-verifier/blob/main/docs/helm-chart-submission.md#provider-controlled-delivery)
