...
from functional.utils import get_name_and_version_from_report
...
def get_run_id(secrets):
    r = github_api('post', f'https://api.github.com/repos/{secrets.test_repo}/pulls/{secrets.pr_number}', secrets.bot_token)
    pr = json.loads(r.text)

    r = github_api('get', f'https://api.github.com/repos/{secrets.test_repo}/actions/runs', secrets.bot_token)
...
def get_run_result(secrets, run_id):
    r = github_api('get', f'https://api.github.com/repos/{secrets.test_repo}/actions/runs/{run_id}', secrets.bot_token)
...
"""
    ...
    github_api('delete', f'https://api.github.com/repos/{test_repo}/git/refs/heads/{pr_base_branch}-gh-pages', bot_token)
    github_api('delete', f'https://api.github.com/repos/{fork_repo}/git/refs/heads/{fork_branch}', bot_token)
...
def the_user_has_created_a_report_without_errors(secrets):
    ...
    r = github_api('get', f'https://api.github.com/repos/{secrets.test_repo}/git/ref/heads/dev-gh-pages', secrets.bot_token)
    j = json.loads(r.text)
    sha = j['object']['sha']

    # Create a new gh-pages branch for testing
    data = {'ref': f'refs/heads/{secrets.pr_base_branch}-gh-pages', 'sha': sha}
    r = github_api('post', f'https://api.github.com/repos/{secrets.test_repo}/git/refs', secrets.bot_token, json=data)
...
def the_user_sends_the_pull_request_with_the_report(secrets):
    ...
    r = github_api('post', f'https://api.github.com/repos/{secrets.test_repo}/pulls', secrets.bot_token, json=data)
    j = json.loads(r.text)
...
def the_user_should_see_the_pull_request_getting_merged(secrets):
    ...
    r = github_api('get', f'https://api.github.com/repos/{secrets.test_repo}/pulls/{secrets.pr_number}/merge', secrets.bot_token)
...
def the_index_yaml_is_updated_with_a_new_entry(secrets):
    ...
    r = github_api('get', f'https://api.github.com/repos/{secrets.test_repo}/releases', secrets.bot_token)
    j = json.loads(r.text)
    ...
    for release in j:
        ...
        if release['tag_name'] == expected_tag:
            ...
            r = github_api('delete', f'https://api.github.com/repos/{secrets.test_repo}/releases/{release_id}', secrets.bot_token)
            if r.status_code != 204:
