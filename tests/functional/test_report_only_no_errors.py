...
import logging
import yaml
from .utils import get_name_and_version_from_report
...
def get_run_id(headers, secrets):
    r = requests.post(
        f'https://api.github.com/repos/{secrets.test_repo}/pulls/{secrets.pr_number}', headers=headers, json={})
    pr = json.loads(r.text)

    r = requests.get(
        f'https://api.github.com/repos/{secrets.test_repo}/actions/runs', headers=headers)
...
@retry(stop_max_delay=60_000*2)
def get_run_result(headers, secrets, run_id):
    r = requests.get(
        f'https://api.github.com/repos/{secrets.test_repo}/actions/runs/{run_id}', headers=headers)
    run = json.loads(r.text)

    if run['conclusion'] is None:
        pytest.fail("Workflow conclusion is null.")
...
vendor:
  ...
  name: ${vendor}
"""
        test_report: str = 'tests/data/report-no-errors.yaml'
    ...
    requests.delete(
        f'https://api.github.com/repos/{test_repo}/git/refs/heads/{pr_base_branch}-gh-pages', headers=headers)
    requests.delete(
        f'https://api.github.com/repos/{fork_repo}/git/refs/heads/{fork_branch}', headers=headers)
...
def the_user_has_created_a_report_without_errors(secrets):
    ...
    headers = {'Accept': 'application/vnd.github.v3+json',
               'Authorization': f'Bearer {secrets.bot_token}'}
    ...
    r = requests.get(
        f'https://api.github.com/repos/{secrets.test_repo}/git/ref/heads/dev-gh-pages', headers=headers)
    j = json.loads(r.text)
    sha = j['object']['sha']
    ...
    r = requests.post(
        f'https://api.github.com/repos/{secrets.test_repo}/git/refs', headers=headers, json=data)
...
def the_user_sends_the_pull_request_with_the_report(secrets):
    ...
    r = requests.post(
        f'https://api.github.com/repos/{secrets.test_repo}/pulls', headers=headers, json=data)
    j = json.loads(r.text)
...
def the_user_should_see_the_pull_request_getting_merged(secrets):
    ...
    run_id = get_run_id(headers, secrets)
    conclusion = get_run_result(headers, secrets, run_id)
    ...
    r = requests.get(
        f'https://api.github.com/repos/{secrets.test_repo}/pulls/{secrets.pr_number}/merge', headers=headers)
...
@then("the index.yaml file is updated with a new entry")
...
@then("a release is published with the chart")
def the_index_yaml_is_updated_with_a_new_entry(secrets):
    ...
    headers = {'Accept': 'application/vnd.github.v3+json',
               'Authorization': f'Bearer {secrets.bot_token}'}
    r = requests.get(
        f'https://api.github.com/repos/{secrets.test_repo}/releases', headers=headers)
    j = json.loads(r.text)
    ...
    for release in j:
        ...
        if release['tag_name'] == expected_tag:
            ...
            logger.info(f"Delete test release '{expected_tag}'")
            release_id = release['id']
            r = requests.delete(
                f'https://api.github.com/repos/{secrets.test_repo}/releases/{release_id}', headers=headers)
            if r.status_code != 204:
